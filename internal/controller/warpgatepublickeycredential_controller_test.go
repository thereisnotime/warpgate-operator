...
var _ = Describe("WarpgatePublicKeyCredential Controller", func() {
	...
	Context("Client error", func() {
		...
		BeforeEach(func() {
			...
			cr := &warpgatev1alpha1.WarpgatePublicKeyCredential{
				...
				Spec: warpgatev1alpha1.WarpgatePublicKeyCredentialSpec{
					ConnectionRef:    "nonexistent-pk-conn",
					Username:         "someuser",
					Label:            "test-key",
	...
	Context("Delete credential with populated credentialID", func() {
		...
		BeforeEach(func() {
			tokenSecret = "pkcred-delpop-token"
			connName = "pkcred-delpop-conn"
			crName = "pkcred-delpop-cred"
			namespace = testNamespace
			deleteCalled = false

			mux := http.NewServeMux()
			mockLogin(mux)
			mux.HandleFunc("/@warpgate/admin/api/users/pre-pk-user/credentials/public-keys/pre-pk-cred", func(w http.ResponseWriter, r *http.Request) {
				if r.Method == http.MethodDelete {
					deleteCalled = true
					w.WriteHeader(http.StatusNoContent)
					return
			...
			cr := &warpgatev1alpha1.WarpgatePublicKeyCredential{
				ObjectMeta: metav1.ObjectMeta{
					Name:       crName,
					Namespace:  namespace,
					Finalizers: []string{publicKeyCredentialFinalizer},
				},
				Spec: warpgatev1alpha1.WarpgatePublicKeyCredentialSpec{
					ConnectionRef:    connName,
					Username:         "someuser",
			...
			// Pre-populate status to simulate a previously-created credential.
			cr.Status.UserID = "pre-pk-user"
			cr.Status.CredentialID = "pre-pk-cred"
	...
	Context("Delete credential with empty credentialID", func() {
		...
		BeforeEach(func() {
			tokenSecret = "pkcred-delempty-token"
			connName = "pkcred-delempty-conn"
			crName = "pkcred-delempty-cred"
			namespace = testNamespace
